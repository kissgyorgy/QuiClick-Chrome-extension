...
class QuiClickAPI {
    ...
    _extensionSettingsToServer(extSettings) {
        const result = {};
        if (extSettings.showTitles !== undefined) result.show_titles = extSettings.showTitles;
        if (extSettings.tilesPerRow !== undefined) result.tiles_per_row = extSettings.tilesPerRow;
        if (extSettings.tileGap !== undefined) result.tile_gap = extSettings.tileGap;
        if (extSettings.showAddButton !== undefined) result.show_add_button = extSettings.showAddButton;
        return result;
    }
    ...
    async getFolderWithBookmarks(folderId) {
        ...
        return {
            ...
            bookmarks: (data.bookmarks || []).map(b => this._serverBookmarkToExtension(b)),
        };
    }
